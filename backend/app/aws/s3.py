...
def upload_model(local_path: Path, trained_at: str) -> bool:
    ...
    Writes to two locations:
      - models/latest/          (always the current production model)
      - models/{trained_at}/    (immutable versioned copy for rollback)

    This is only called from local training or CI — production training is
    blocked at the API level (APP_ENV=production returns 403 on /api/ml/train),
    so this function always receives a full-dataset model.
